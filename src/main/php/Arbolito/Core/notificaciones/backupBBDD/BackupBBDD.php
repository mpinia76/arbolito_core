<?php
namespace Arbolito\Core\notificaciones\backupBBDD;

use Arbolito\Core\utils\XTemplate;
...
class BackupBBDD{
	...
	function backupDatabaseTables($dbHost,$dbUsername,$dbPassword,$dbName, $filename,$tables = '*' ){
	    ...
	    //save file
		$nameFile ='db_Arbolito_' . date("Y-m-d-H-i-s") .'.sql';
	...
	public function send(){
		...
		$this->setPathBackup(ArbolitoConfig::getInstance()->getAppPathCore()."/backup/");

		$filename = 'db_arbolito_' . date("Y-m-d-H-i-s") . '.zip';
		$backupFile = $this->getPathBackup().$filename;

		self::backupDatabaseTables(ArbolitoConfig::getInstance()->getDbHost(),ArbolitoConfig::getInstance()->getDbUser(),ArbolitoConfig::getInstance()->getDbPassword(),ArbolitoConfig::getInstance()->getDbName(),$filename);
		...
		$subjectMail = 'BackUp cose_arbolito';
		...
		$template->assign("titulo", "En el archivo adjunto se encuentra el BackUp de la BBDD cose_arbolito realizado el ".date('d/m/Y H:i:s'));
		...
		ArbolitoUtils::sendMail(ArbolitoConfig::TEST_MAIL_TO,ArbolitoConfig::TEST_MAIL_TO, $subjectMail, $mensaje, $attachs);
		unlink($backupFile);
