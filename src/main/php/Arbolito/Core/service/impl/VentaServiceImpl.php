...
use Arbolito\Core\criteria\MovimientoVentaCriteria;
...
class VentaServiceImpl extends CrudService implements IVentaService {
	...
	function validateOnAdd( $entity ){
        ...
        foreach ($entity->getDetalles() as $detalle) {
            if (!$detalle->getProducto()->getTipoProducto()->getStock()){
                $criteria = new VentaCriteria();
                $criteria->setCliente($entity->getCliente());
                $oVentas = $this->getDAO()->getList( $criteria );
                foreach ($oVentas as $oVenta) {
                    if($oVenta->podesAnularte()){
                        foreach ($oVenta->getDetalles() as $detalleVenta) {
                            if ($detalle->getProducto()->getOid()==$detalleVenta->getProducto()->getOid()){
                                throw new ServiceException("venta.detalles.repetido");
                            }
                        }
                    }
                }
            ...
            //Logger::log('Cliente: '.$entity->getCliente()->getOid().' - Producto: '.$detalle->getProducto()->getOid());
        }
	...
	/**
	 * (non-PHPdoc)
	 * @see src/main/php/Arbolito/Core/service/Arbolito\Core\service.IVentaService::cobrar()
	 */
	public function cobrar(Venta $venta, Cuenta $cuenta, User $user, $montoPagar, $montoActualizado){
		...
		$movimiento->setConcepto( ArbolitoUtils::getConceptoMovimientoVenta() );
	...
	public function anular(Venta $venta, User $user){
		...
		if( $venta->getMonto() > $venta->getMontoDebe() ){
			...
			foreach ($movimientos as $movimiento) {
				...
				$contramovimiento->setConcepto( ArbolitoUtils::getConceptoMovimientoAnulacionVenta() );
...
/**
	 * (non-PHPdoc)
	 * @see src/main/php/Arbolito/Core/service/Arbolito\Core\service.IVentaService::devolver()
	 */
	public function devolver(Venta $venta, Cuenta $cuenta, User $user){
		...
		if( $venta->getMontoDevolucion() > $venta->getMontoDebe() ){
			...
			$movimiento->setConcepto( ArbolitoUtils::getConceptoMovimientoDevolucion() );
			$movimiento->setUser($user);
	...
	/**
	 * (non-PHPdoc)
	 * @see src/main/php/Arbolito/Core/service/Arbolito\Core\service.IVentaService::cobrarCtaCte()
	 */
	...
	/**
	 * (non-PHPdoc)
	 * @see src/main/php/Arbolito/Core/service/Arbolito\Core\service.IVentaService::getTotalesDia()
	 */
	...
	/**
	 * (non-PHPdoc)
	 * @see src/main/php/Arbolito/Core/service/Arbolito\Core\service.IVentaService::getTotalesMes()
	 */
    ...
    /**
     * (non-PHPdoc)
     * @see src/main/php/Arbolito/Core/service/Arbolito\Core\service.IVentaService::getUltimoComprobante()
     */
    public function getUltimoComprobante(  ){

        $criteria = new VentaCriteria();

        $criteria->addOrder("nroComprobante", "DESC");
        $criteria->setMaxResult(1);

        try{
            $venta = $this->getSingleResult( $criteria );
            return $venta->getNroComprobante();


        }catch (ServiceNonUniqueResultException $ex){
            throw new ServiceException("producto.codigo.invalido");
